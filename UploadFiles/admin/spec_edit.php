...
if($dopost!='save')
{
    ...
    $tags = GetTags($aid);
    include MiInclude("templets/spec_edit.htm");
}
/*--------------------------------
function __save(){  }
-------------------------------*/
else if($dopost=='save')
{
    ...
    for($i=1;$i<=$cfg_specnote;$i++)
    {
        if(!empty(${'notename'.$i}))
        {
            ...
            $notelist .= "{missra:specnote imgheight=\\'$imgheight\\' imgwidth=\\'$imgwidth\\'
    ...
    //返回成功信息
    $msg = "　　请选择你的后续操作：
    <a href='spec_add.php?cid=$typeid'>发布新专题</a>
    &nbsp;&nbsp;
    <a href='archives_do.php?aid=".$id."&dopost=editArchives'>返回更改</a>
    &nbsp;&nbsp;
    <a href='$artUrl' target='_blank'>查看专题</a>
    &nbsp;&nbsp;
    <a href='content_s_list.php'>已发布专题管理</a> ";
    $wintitle = "成功更改一个专题！";
