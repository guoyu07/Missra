...
if($action == 'edit' || $action == 'newfile')
{
    ...
    //读取文件内容
    //$content = mi_htmlspecialchars(trim(file_get_contents($truePath.$filename)));
    if($action == 'edit')
    {
    ...
    foreach($dtags as $tag)
    {
        ...
        fclose($fp);
        $helps[$tag] = explode('>>missra>>', $helpContent);
    }

    include MIADMIN.'/templets/tpl_edit.htm';
    exit();
}
/*---------------------------
function save_tpl() { }
...
else if ($action == 'uploadok')
{
    ...
    else
    {
        ...
        {
            ShowMsg("Missra模板只能用 .htm 或 .html扩展名！", "-1");
          exit();
        }
...
?".'>';
    ...
    else
    {
        ...
        $fp = fopen(MIINC.'/taglib/'.$filename,'r');
        $democode = fread($fp, filesize(MIINC.'/taglib/'.$filename));
        fclose($fp);
        $title = "修改标签";
    }
    include MIADMIN.'/templets/tpl_edit_tag.htm';
    exit();
}
/*---------------------------
...
else if($action=='savetagfile')
{
    ...
    $content = stripslashes($content);
    $truefile = MIINC.'/taglib/'.$filename;
    $fp = fopen($truefile, 'w');
    fwrite($fp, $content);
    fclose($fp);
    $msg = "
    <form name='form1' action='tag_test_action.php' target='blank' method='post'>
      <input type='hidden' name='dopost' value='make' />
        ...
        <textarea name='partcode' cols='150' rows='6' style='width:90%;'>{missra:{$tagname} }{/missra:{$tagname}}</textarea><br />
