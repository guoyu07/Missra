...
else if($dopost=="export")
{
    ...
    $row = $dsql->GetOne("SELECT * FROM `#@__channeltype` WHERE id='$id' ");
    $channelconfig = '';
    $row['maintable'] = preg_replace('#mi_#', '#@__', $row['maintable']);
    $row['addtable'] = preg_replace('#mi_#', '#@__', $row['addtable']);
    foreach($row as $k=>$v)
    {
        if($k=='fieldset') $v = "\r\n$v\r\n";
...
{
    require_once(MIADMIN."/inc/inc_admin_channel.php");
    ...
    if(!is_array($dtp->CTags)) GotoStaMsg("模型规则不是合法的Missra模型规则！");
    ...
    {
        GotoStaMsg("模型规则不是合法的Missra模型规则！");
    }
...
else if($dopost == 'modifysearch')
{
    ...
    } else if ($step == 1)
    {
        ...
        $dsql->ExecuteNoneQuery($query);
        $formshtml = mi_htmlspecialchars($forms);
...
$row = $dsql->GetOne("SELECT * FROM `#@__channeltype` WHERE id='$id' ");
require_once(MIADMIN."/templets/mychannel_edit.htm");