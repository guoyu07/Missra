	...
	function IsWritable($pathfile) {
		...
		if ($isDir) {
			if (is_dir($pathfile)) {
				...
				$pathfile = $pathfile.'mi_'.uniqid(mt_rand()).'.tmp';
...
// 检测权限
$safeMsg = array();
//if(TestExecuteable(MIROOT.'/data',$cfg_basehost) || TestExecuteable(MIROOT.'/uploads',$cfg_basehost)) {
//	$helpurl = "http://help.missra.com/";
...
if(IsWritable(MIDATA.'/common.inc.php')) {
...
//if(PostHost($cfg_basehost.'/data/admin/ver.txt') === @file_get_contents(MIDATA.'/admin/ver.txt')) {
//	$helpurl = 'http://help.missra.com/';
...
<div id="safemsg">
	<dl class="dbox" id="item1">
    	<dt class='lside'>
			<span class='l'><?php echo $cfg_soft_enname; ?>安全提示</span>
		</dt>
    	<dd>
			<ul>
				<?php $i=1; foreach($safeMsg as $key => $val){ ?>
					<li class="nline abc"><font color="black"><?php echo $i;?> . </font> <font color="red"><?php echo $val;?></font></li>
				<?php $i++; } ?>
			</ul>
    	</dd>
  	</dl>
