...
//生成必须同步服务器的列表
if(file_exists(MIDATA.'/config.file.inc.php'))
{
    require_once(MIDATA.'/config.file.inc.php');
}
...
{
    //将其转换为配置写入文件
    $configfile = MIDATA.'/config.file.inc.php';
...
else if($dopost == 'make')
{
    if($step == 1)
    {
        ...
        $cacheMakeFile = MIDATA.'/cache/filelist.inc.php';
        ...
        if(count($Iterm) > 0)
        {
            ...
            foreach($Iterm as $key => $val)
            {
                ...
                if(is_array($dirlist = getDirs(MIROOT.$val)))
                {
    ...
    {
        if(file_exists(MIDATA.'/cache/filelist.inc.php'))
        {
            require_once(MIDATA.'/cache/filelist.inc.php');
        }
        if(is_array($dirlist))
        {
            if($sta > 0 && $sta < $totalnum)
            {
                ...
                if($ftp->connect($config))
                {
                    ...
                    {
                        //如果是文件目录
                        $remotedir = str_replace(MIROOT, '', $dirname).'/';
                        $localdir = '..'.$remotedir.'/';
                        ...
                        $remotefile = str_replace(MIROOT, '', $dirname);
                        ...
                        {
                            list($filename,$fileconfig) = explode('|', $val); 
                            if(preg_replace('/[^\/]*\.(\w){0,}/', '', str_replace(MIROOT, '', $filename)) == $remotedir)
                            {
                ...
                @unlink(MIDATA.'/cache/filelist.inc.php');
                echo '全部同步完毕!';exit;
            }
        } else {
...
include MiInclude('templets/makeremote_all.htm');