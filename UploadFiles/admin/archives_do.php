...
else if($dopost=="viewArchives")
{
    ...
    if($trow['issystem'] != -1)
    {
		...
		PutCookie('MiUserID',$arcRow['mid'],1800);
		PutCookie('MiLoginTime',time(),1800);
        if($arcRow['ismake']==-1 || $arcRow['corank']!=0 || $arcRow['arcrank']!=0 || ($arcRow['typeid']==0 && $arcRow['channel']!=-1) || $arcRow['money']>0)
        {
            echo "<script type='text/javascript'>location.href='{$cfg_phpurl}/view.php?aid={$aid}';</script>";
            exit();
        }
    }
    else
    ...
    echo "<script type='text/javascript'>location.href='$arcurl"."?".time()."';</script>";
    exit();
}
/*--------------------------
//异步上传缩略图
...
else if($dopost=='moveArchives')
{
    ...
    if(empty($totype))
    {
        ...
        echo "</div>\r\n";
        echo "<form name='quickeditform' action='archives_do.php' method='post' class='quickeditform'>\r\n";
...
else if($dopost=='quickEdit')
{
    ...
    echo "    <div class='titRight'><img src='images/ico-close.gif' style='cursor:pointer;' onclick='HideObj(\"{$divname}\");ChangeFullDiv(\"hide\");' alt='关闭' title='关闭' /></div>\r\n";
    echo "</div>\r\n";
    echo "<form name='quickeditform' action='archives_do.php?dopost=quickEditSave&aid={$aid}' method='post' class='quickeditform'>\r\n";
...
else if($dopost=='quickEditSave')
{
    ...
    $title = mi_htmlspecialchars(cn_substrR($title, $cfg_title_maxlen));
...
{
    include_once(MIINC.'/splitword.class.php');
    CheckPurview('a_Commend,sys_ArcBatch');
    if( !empty($aid) && empty($qstr) ) $qstr = $aid;
...
{
    require_once(MIINC.'/typeunit.class.selector.php');
    AjaxHead();
    //输出AJAX可移动窗体
    $divname = 'getCatMap';
