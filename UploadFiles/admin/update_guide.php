...
else if($dopost=='getlist')
{
    ...
    else
    {
        ...
        $allFileList .= "<div class='upinfotitle'>以下是需要下载的更新文件（路径相对于Missra的根目录）：</div>\r\n";
        $filelists = explode("\n",$filelist);
    ...
    include MiInclude('templets/update_guide_getlist.htm');
    exit();
}
/**
下载文件（保存需下载内容列表）
...
{
    $cacheFiles = MIDATA.'/cache/updatetmp.inc';
    ...
    include MiInclude('templets/update_guide_getfiles.htm');
    exit();
}
/**
下载文件，具体操作步骤
...
else if($dopost=='down')
{
    ...
    else
    {
        ...
        $dhd->Close();
        $truefile = MIDATA.'/'.$tmpdir.'/sql.txt';
        $fp = fopen($truefile, 'w');
        fwrite($fp, $ct);
        fclose($fp);
