 ...
 */
define('MIADMIN', str_replace("\\", '/', dirname(__FILE__) ) );
require_once(MIADMIN.'/../include/common.inc.php');
...
if(file_exists(MIDATA.'/admin/skin.txt'))
{
	$skin = file_get_contents(MIDATA.'/admin/skin.txt');
	$cfg_admin_skin = !in_array($skin, array(1,2,3,4))? 1 : $skin;
}

//获得当前脚本名称，如果你的系统被禁用了$_SERVER变量，请自行更改这个选项
$miNowurl = $s_scriptName = '';
...
{
    header("location:login.php?gotopage=".urlencode($miNowurl));
    exit();
}
...
function XSSClean($val)
{
    ...
    if (is_array($val))
    {
        ...
        {
            if(in_array($key,array('tags','body','mi_fields','mi_addonfields','dopost','introduce'))) continue;
...
if($cfg_mi_log=='Y')
{
    ...
    $s_query = isset($miNowurls[1]) ? $miNowurls[1] : '';
...
//管理缓存、管理员频道缓存
$cache1 = MIDATA.'/cache/inc_catalog_base.inc';
if(!file_exists($cache1)) UpDateCatCache();
$cacheFile = MIDATA.'/cache/admincat_'.$cuserLogin->userID.'.inc';
if(file_exists($cacheFile)) require_once($cacheFile);

//更新服务器
require_once (MIDATA.'/admin/config_update.php');
...
if(strlen($cfg_cookie_encode)<=10)
{
	...
	$configfile = MIDATA.'/config.cache.inc.php';
...
function UpDateCatCache()
{
    ...
    $cache2 = MIDATA.'/cache/channelsonlist.inc';
    $cache3 = MIDATA.'/cache/channeltoplist.inc';
...
{
    global $dsql;
    $cachefile = MIDATA.'/cache/member_model.inc';
 ...
 * @return    string
 */
function MiInclude($filename, $isabs=FALSE)
{
    return $isabs ? $filename : MIADMIN.'/'.$filename;
}
