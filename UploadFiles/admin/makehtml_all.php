...
if($action=='')
{
    require_once(MIADMIN."/templets/makehtml_all.htm");
    exit();
}
else if($action=='make')
{
    ...
    {
        include_once(MIADMIN."/makehtml_archives_action.php");
        exit();
    }
    //更新主页
    /*-------------------------
    ...
    if($step==3)
    {
        include_once(MIINC."/arc.partview.class.php");
        $pv = new PartView();
        $row = $pv->dsql->GetOne("SELECT * FROM `#@__homepageset` ");
		$templet = str_replace("{style}", $cfg_df_style,$row['templet']);
		$homeFile = MIADMIN.'/'.$row['position'];
    ...
    else if($step==4)
    {
        ...
        $mkcachefile = MIDATA."/mkall_cache_{$adminID}.php";
