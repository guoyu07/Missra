...
if($dopost!='save')
{
    ...
    include MiInclude("templets/soft_edit.htm");
    exit();
}
/*--------------------------------
function __save(){  }
...
else if($dopost=='save')
{
    ...
    for($i=1; $i<=30; $i++)
    {
        if(!empty(${'softurl'.$i}))
        {
            ...
            if($softurl != 'http://')
            {
                if($islocal==1) $urls .= "{missra:link islocal='$islocal' text='{$servermsg}'} $softurl {/missra:link}\r\n" ;
                else if($isneed) $urls .= "{missra:link text='$servermsg'} $softurl {/missra:link}\r\n";
                else continue;
            }
        }
    }
