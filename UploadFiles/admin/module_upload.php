...
if($action=='upload') {
    if( !is_uploaded_file($upfile) ) {
        ...
        if($filetype==1) {
            ...
            $mifileindex = -1;
            //为了节省资源，系统仅以.xml作为扩展名识别ZIP包里了missra模块格式文件
            if(is_array($files)) {
                for($i=0; $i<count($files); $i++) {
                    if( preg_match( "#\.xml#i", $files[$i]['filename']) )
                    {
                        $mifile = $files[$i]['filename'];
                        $mifileindex = $i;
                        break;
                    }
                }
            ...
            if($mifileindex==-1) {
                unlink($tmpfilename);
                ShowMsg("对不起，你上传的压缩包中不存在missra模块文件！<br /><br /><a href='javascript:history.go(-1);'>&gt;&gt;返回重新上传&gt;&gt;</a>","javascript:;");
                exit();
            }
            $ziptmp = $mdir.'/ziptmp';
            $z->Extract($tmpfilename, $ziptmp, $mifileindex);
            unlink($tmpfilename);
            $tmpfilename = $mdir."/ziptmp/".$mifile;
        }
        ...
        if(empty($infos['hash'])) {
            unlink($tmpfilename);
            $dm->Clear();
            ShowMsg("对不起，你上传的文件可能不是玫莎模块的标准格式文件！<br /><br /><a href='javascript:history.go(-1);'>&gt;&gt;返回重新上传&gt;&gt;</a>","javascript:;");
            exit();
        }
    ...
    $msg = "
        <table width='600' border='0' cellspacing='0' cellpadding='0'>
            ...
            <tr>
                ...
                <td><input name='delhas' type='checkbox' id='delhas' value='1' /> 强制删除同名模块(这可能导致已经安装的模块无法卸载)</td>
            </tr>
            <tr>
