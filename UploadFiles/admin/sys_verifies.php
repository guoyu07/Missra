...
@set_time_limit(0);
require(MIINC.'/mihttpdown.class.php');
...
//当前软件指纹码锁定文件
$verifiesLockFile = MIDATA.'/admin/verifies.txt';
...
if($action == '')
{
    include(MIADMIN.'/templets/sys_verifies.htm');
    exit();
}
/*----------------
校验文件
...
else if($action == 'verify')
{
    ...
    {
        $turefile = str_replace('../admin', '.', $row['filename']);
        //跳过不存在的文件
    ...
    {
        include(MIADMIN.'/templets/sys_verifies_verify.htm');
    }
    exit();
}
/*--------------------
...
else if ($action == 'manage')
{
    ...
    include(MIADMIN.'/templets/sys_verifies_manage.htm');
    exit();
}
/*-----------------------
下载文件
...
else if ($action == 'getfiles')
{
    ...
    $cacheFiles = MIDATA.'/modifytmp.inc';
    ...
    foreach($refiles as $filename)
    {
        ...
        {
            $curdir = GetDirName( preg_replace("#^admin/#i", $adminDir.'/', $filename) );
        } else {
    ...
    include(MIADMIN.'/templets/sys_verifies_getfiles.htm');
...
else if($action=='down')
{
    ...
    $dhd->SaveToBin(MIDATA.'/'.$tmpdir.'/'.$files[$curfile]);
    $dhd->Close();
...
else if($action=='modify')
{
    ...
    else
    {
        foreach($modifys as $fname)
        {
            if($method=='local')
            {
                $tureFilename = str_replace('../admin','./',$fname);
...
else if ($action == 'apply')
{
    ...
    if(isset($files) && is_array($files))
    {
        foreach($files as $f)
        {
            if(preg_match("#^admin#", $f)) $tf = preg_replace("#^admin#", $adminDir, $f);
            else $tf = $f;
