...
if($dopost!='save')
{
    ...
    include MiInclude("templets/album_edit.htm");
    exit();
}
/*--------------------------------
function __save(){  }
...
else if($dopost=='save')
{
    ...
    $imgurls = "{missra:pagestyle maxwidth='$maxwidth' pagepicnum='$pagepicnum' ddmaxwidth='$ddmaxwidth' row='$row' col='$col' value='$pagestyle'/}\r\n";
    $hasone = false;
    ...
    for($i=1; $i<=120; $i++)
    {
        ...
        if( isset(${'imgfile'.$i}) && is_uploaded_file(${'imgfile'.$i}) )
        {
            ...
            if(!is_array($imginfos))
            {
                ...
                $imgurls .= "{missra:img ddimg='$ddurl' text='$iinfo' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/missra:img}\r\n";
                continue;
            }
            ...
            $imgurls .= "{missra:img ddimg='$litpicname' text='$iinfo' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/missra:img}\r\n";
        }
        //没上传图片(只修改msg信息)
        else
        {
    ...
    if($formzip==1)
    {
        include_once(MIINC."/zip.class.php");
        include_once(MIADMIN."/file_class.php");
        $zipfile = $cfg_basedir.str_replace($cfg_mainsite,'',$zipfile);
        $tmpzipdir = MIDATA.'/ziptmp/'.cn_substr(md5(ExecTime()),16);
        $ntime = time();
        if(file_exists($zipfile))
        {
            ...
            foreach($imgs as $imgold)
            {
                ...
                if(is_file($imgfile))
                {
                    ...
                    $info = '';
                    $imginfos = GetImageSize($imgfile,$info);
                    $imgurls .= "{missra:img ddimg='$litpicname' text='' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/missra:img}\r\n";
    ...
    if(is_array($_SESSION['bigfile_info']))
    {
        foreach($_SESSION['bigfile_info'] as $k=>$v)
        {
            ...
            $imgurls .= "{missra:img ddimg='$litpicname' text='$imginfo' width='".$imginfos[0]."' height='".$imginfos[1]."'} $v {/missra:img}\r\n";
        }
    }
    ...
    $msg =
    " 　　请选择你的后续操作：
    <a href='album_add.php?cid=$typeid'>发布新图集</a>
    &nbsp;&nbsp;
    <a href='archives_do.php?aid=".$id."&dopost=editArchives'>返回更改</a>
    &nbsp;&nbsp;
    <a href='$arcUrl' target='_blank'>预览文档</a>
    &nbsp;&nbsp;
    <a href='catalog_do.php?cid=$typeid&dopost=listArchives'>管理已发布图片</a>
    &nbsp;&nbsp;
    $backurl
    ";
