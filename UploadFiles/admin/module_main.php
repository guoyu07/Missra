...
require_once(dirname(__FILE__)."/../include/mimodule.class.php");
require_once(dirname(__FILE__)."/../include/oxwindow.class.php");
if(empty($action)) $action = '';
require_once(MIDATA."/admin/config_update.php");
$mdir = MIDATA.'/module';
$mdurl = $updateHost.'missra/module_'.$cfg_soft_lang.'/modulelist.txt';
...
else if($action=='setup')
{
    ...
    $prvdir .= "<tr><th width='270'>目录</td><th align='center'>可写</td></tr>\r\n";
...
else if($action=='setupstart')
{
    ...
    {
        include(MIDATA.'/module/'.$filename);
        exit();
    }
    //系统自动安装
    else
    {
        ...
        $rflwft = "<script type='text/javascript'>\r\n";
...
else if($action=='uninstallok')
{
    ...
    {
        include(MIDATA."/module/{$hash}-uninstall.php");
        @unlink(MIDATA."/module/{$hash}-uninstall.php");
        exit();
    }
    else
    {
...
else if($action=='view')
{
    ...
    if(file_exists(MIDATA."/module/{$hash}-readme.php")) 
    {
...
else if($action=='edit')
{
    ...
    $setupsql40 = mi_htmlspecialchars($dm->GetSystemFile($hash, 'setupsql40'));
...
{
	$model_remote_url = $updateHost.'missra/module_'.$cfg_soft_lang.'/'.$hash.'.xml';
