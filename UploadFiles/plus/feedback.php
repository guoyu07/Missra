...
require_once(MIINC."/filter.inc.php");
if(!isset($action))
{
    $action = '';
}
...
include_once(MIINC."/memberlogin.class.php");
...
else if($action=='' || $action=='show')
{
    ...
    $dlist->SetTemplate(MITEMPLATE.'/plus/feedback_templet.htm');
...
else if($action=='quote')
{
    ...
    $dtp = new MiTemplate();
	$tplfile = $type == ''? MITEMPLATE.'/plus/feedback_quote.htm' : MITEMPLATE.'/plus/feedback_quote_ajax.htm';
...
else if($action=='send')
{
    ...
    if($isconfirm!='yes' && $cfg_feedback_ck=='Y')
    {
        ...
        require_once(MIINC.'/mitemplate.class.php');
        $dtp = new MiTemplate();
        $dtp->LoadTemplate(MITEMPLATE.'/plus/feedback_confirm.htm');
        $dtp->Display();
        exit();
    }
    //检查验证码
