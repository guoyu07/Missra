...
{    
   ...
   VALUES ('$buyid', '$pname', '$product' , '$price' , '$mtime' , '$pid' , '$mid' , '0' , '$ptype');
    ...
    $tpl = new MiTemplate();
    $tpl->LoadTemplate(MIMEMBER.'/templets/buy_action_payment.htm');
    $tpl->Display();
    ...
    if (!file_exists(MIINC.'/payment/'.$rs['code'].'.php')) {
        ShowMsg("未发现支付接口文件，请到后台配置！",'javascript:;');
        exit();
    }

    require_once MIINC.'/payment/'.$rs['code'].'.php';
    $pay = new $rs['code'];
    $payment="";
    if($rs['code']=="cod" || $rs['code']=="bank") {
        ...
        require_once MIDATA.'/payment/'.$rs['code'].'.php';
    }
    ...
    $dtp->LoadTemplate(MIMEMBER.'/templets/shops_action_payment.htm');
    $dtp->Display();
    exit();
}
