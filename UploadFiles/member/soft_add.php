...
if(empty($dopost))
{
    ...
    include(MIMEMBER."/templets/soft_add.htm");
    exit();
}
...
{
    $description = '';
    include(MIMEMBER.'/inc/archives_check.php');
...
'$color','$writer','$source','$litpic','$pubdate','$senddate','$mid','$description','$keywords'); ";
    ...
    $softurl1 = str_replace(array("{missra:","{/missra:","}"), "#", $softurl1);
    $urls = '';
    if($softurl1!='')
    {
        //$urls .= "{missra:link islocal='1' text='{$servermsg1}'} $softurl1 {/missra:link}\r\n";
		if (preg_match("#}(.*?){/missra:link}{missra:#sim", $servermsg1) != 1) {
			$urls .= "{missra:link islocal='1' text='{$servermsg1}'} $softurl1 {/missra:link}\r\n";
		} 
    }
    for($i=2; $i<=12; $i++)
    {
        if(!empty(${'softurl'.$i}))
        {
			...
			$softurl = str_replace(array("{missra:","{/missra:","}"), "#", $softurl);
            if($servermsg=='')
            {
                $servermsg = '下载地址'.$i;
            }
            ...
            {
                $urls .= "{missra:link text='$servermsg'} $softurl {/missra:link}\r\n";
            }
        }
    }
    ...
    #api{{
    if(defined('UC_API') && @include_once MIROOT.'/api/uc.func.php')
    {
        //推送事件
        $feed['icon'] = 'thread';
