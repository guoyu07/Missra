 ...
 * 文档处理函数
 * 
 * @version        $Id: inc_archives_functions.php
 * @package        Mi.Member
 * @copyright      Copyright (c)  2010, Missra
 * @license        http://help.missra.com/license.html
 * @link           http://www.missra.com
 */
if(!defined('MIMEMBER')) exit('missra');
require_once(MIINC.'/image.func.php');
require_once(MIINC.'/archives.func.php');
...
function GetCurContentAlbum($body,$rfurl,&$firstdd)
{
    ...
    include_once(MIINC."/micollection.func.php");
    ...
    foreach($img_array as $key => $value)
    {
        ...
        if($cfg_mb_rmdown=='Y')
        {
            ...
            if($rs)
            {
                ...
                $rsimg .= "{missra:img ddimg='$litpicname' text='' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/missra:img}\r\n";
            }
        }
        else
        {
            $rsimg .= "{missra:img ddimg='$value' text='' width='0' height='0'} $value {/missra:img}\r\n";
        }
    }
    return $rsimg;
}
...
function PrintAutoFieldsAdd(&$fieldset, $loadtype='all', $isprint=TRUE)
{
    ...
    if(is_array($dtp->CTags))
    {
        foreach($dtp->CTags as $tid=>$ctag)
        {
            ...
            {
                $mi_addonfields .= ( $mi_addonfields=="" ? $ctag->GetName().",".$ctag->GetAtt('type') : ";".$ctag->GetName().",".$ctag->GetAtt('type') );
                $addonfieldsname .= ",".$ctag->GetName();
                if ($isprint) echo  GetFormItemA($ctag);
            }
        }
    }
    if ($isprint) echo "<input type='hidden' name='mi_addonfields' value=\"".$mi_addonfields."\">\r\n";
    echo "<input type=\"hidden\" name=\"mi_fieldshash\" value=\"".md5($mi_addonfields.$cfg_cookie_encode)."\" />";
    // 增加一个返回
...
function PrintAutoFieldsEdit(&$fieldset, &$fieldValues, $loadtype='all')
{
    ...
    echo "<input type='hidden' name='mi_addonfields' value=\"".$mi_addonfields."\">\r\n";
}
...
function MakeArt($aid, $ismakesign=FALSE)
{
    ...
    include_once(MIINC.'/arc.archives.class.php');
    if($ismakesign)
    {
        $envs['makesign'] = 'yes';
    }
