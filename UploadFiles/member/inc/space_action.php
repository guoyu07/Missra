<?php   if(!defined('MIMEMBER')) exit('missra');
/**
 * 空间操作
 * 
 * @version        $Id: space_action.php
 * @package        Mi.Member
...
if($action=='article')
{
    ...
    include_once(MIINC.'/arc.memberlistview.class.php');
    include_once(MIINC.'/channelunit.func.php');
    ...
    $dlist->SetTemplate(MIMEMBER."/space/{$_vars['spacestyle']}/listarticle.htm");
...
else if($action=='viewarchives' && !empty($aid) && is_numeric($aid))
{
    ...
    //解析模板
    $dlist = new MemberListview();
    $dlist->SetTemplate(MIMEMBER."/space/{$_vars['spacestyle']}/blog.htm");
    $dlist->Display();
    exit();
}
/*---------------------------------
...
else if($action=='archives')
{
    ...
    $dlist->SetParameter("action", $action);
    $dlist->SetTemplate(MIMEMBER."/space/{$_vars['spacestyle']}/listarchives.htm");
    $dlist->SetSource($query);
    $dlist->Display();
    exit();
}
...
else if($action=='album')
{
    ...
    $dlist->SetParameter("uid",$_vars['userid']);
    $dlist->SetParameter("action",$action);
    $dlist->SetTemplate(MIMEMBER."/space/{$_vars['spacestyle']}/listalbum.htm");
    $dlist->SetSource($query);
    $dlist->Display();
    exit();
}
...
else if($action=='guestbook')
{
    ...
    $dlist->SetParameter("uid",$_vars['userid']);
    $dlist->SetParameter("action",$action);
    $dlist->SetTemplate(MIMEMBER."/space/{$_vars['spacestyle']}/guestbook.htm");
    $dlist->SetSource($query);
    $dlist->Display();
    exit();
}
...
else if($action=='friend')
{
    ...
    $dlist->SetParameter("uid",$_vars['userid']);
    $dlist->SetParameter("action",$action);
    $dlist->SetTemplate(MIMEMBER."/space/{$_vars['spacestyle']}/friend.htm");
    $dlist->SetSource($query);
    $dlist->Display();
    exit();
}
...
{
    include_once(MIDATA.'/enums/nativeplace.php');
    include_once(MIINC."/enums.func.php");
    $row = $dsql->GetOne("SELECT  * from `#@__member_person` where mid='{$_vars['mid']}' ");
    $dpl = new MiTemplate();
    $dpl->LoadTemplate(MIMEMBER."/space/{$_vars['spacestyle']}/infos.htm");
    $dpl->display();
}
...
else if($action=='delfriend')
{
    ...
    else
    {
        ...
        #/aip}}
        $inquery = "DELETE FROM `mi_member_friends` where fid='{$_vars['mid']}' And mid='{$cfg_ml->M_ID}' ";
        $dsql->ExecuteNoneQuery($inquery);
        //统计我的好友数量
        $row = $dsql->GetOne("SELECT COUNT(*) AS nums FROM `#@__member_friends` WHERE `mid`='".$cfg_ml->M_ID."'");
        $dsql->ExecuteNoneQuery("UPDATE `#@__member_tj` SET friend='$row[nums]' WHERE `mid`='".$cfg_ml->M_ID."'");
...
{
    $dpl = new MiTemplate();
    $dpl->LoadTemplate(MIMEMBER."/space/{$_vars['spacestyle']}/introduce.htm");
    $dpl->display();
}
//联系我们
...
{
    $dpl = new MiTemplate();
    $dpl->LoadTemplate(MIMEMBER."/space/{$_vars['spacestyle']}/contact.htm");
    $dpl->display();
}
/*-------------------------------
function products() { }
...
elseif($action == 'products')
{
    ...
    $dlist->SetParameter('uid', $_vars['userid']);
    $dlist->SetParameter('action', $action);
    $dlist->SetTemplate(MIMEMBER."/space/{$_vars['spacestyle']}/listproducts.htm");
    $dlist->SetSource($query);
    $dlist->Display();
    exit();
}