...
if($step == 1)
{
    ...
    if($dopost=='regbase')
    {
       ...
       '0','$spaceSta','','$safequestion','$safeanswer','$jointime','$joinip','$logintime','$loginip'); ";
        if($dsql->ExecuteNoneQuery($inQuery))
        {
            ...
            //写入其它默认数据
            $dsql->ExecuteNoneQuery("INSERT INTO `#@__member_flink`(mid,title,url) VALUES('$mid','玫莎内容管理系统','http://www.missra.com'); ");
            ...
            if($cfg_mb_spacesta==-10)
            {
                ...
                $mailbody .= "{$url}\r\n\r\n";
                $mailbody .= "Power by http://www.missra.com 玫莎内容管理系统！\r\n";
            ...
            if($cfg_mb_reginfo == 'Y' && $spaceSta >=0)
            {
                ...
                require_once(MIMEMBER."/templets/reg-new3.htm");
                exit;
            } 
        } else {
    ...
    require_once(MIMEMBER."/templets/reg-new.htm");
} else {
    if(!$cfg_ml->IsLogin())
    {
    ...
    require_once(MIMEMBER."/templets/reg-new2.htm");
}