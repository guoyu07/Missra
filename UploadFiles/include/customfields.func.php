...
/**
 * 系统核心函数存放文件
 * @version        $Id: customfields.func.php
...
function GetFormItem($ctag, $admintype='admin')
{
    ...
    else if($fieldType=='stepselect')
    {
            ...
            {
                $myformItem .= '<script type="text/javascript" src="'.$cmspath.'images/js/enums.js"></script>'."\r\n";
                $GLOBALS['hasSetEnumJs'] = 'hasset';
            }
            $myformItem .= "<script type='text/javascript' src='{$cmspath}/data/enums/{$fieldname}.js'></script>\r\n";
            $myformItem .= '<script type="text/javascript">MakeTopSelect("'.$fieldname.'", 0);</script>'."\r\n";
...
function GetFieldValue($dvalue, $dtype, $aid=0, $job='add', $addvar='', $admintype='admin', $fieldname='')
{
    ...
    else if($dtype=='img' || $dtype=='imgfile')
    {
        ...
        $imgurl = "{missra:img text='' width='' height=''} ".$iurl." {/missra:img}";
        if(preg_match("/^http:\/\//i", $iurl) && $GLOBALS['cfg_isUrlOpen'])
        {
            ...
            if($GLOBALS['cfg_isUrlOpen'])
            {
                ...
                if(is_array($reimgs))
                {
                    ...
                    {
                        $imgurl = "{missra:img text='' width='".$reimgs[1]."' height='".$reimgs[2]."'} ".$reimgs[0]." {/missra:img}";
                    }
                }
            }
            else
        ...
        else if($iurl != '')
        {
            ...
            if(is_file($imgfile))
            {
                ...
                {
                    $imgurl = "{missra:img text='' width='".$imginfos[0]."' height='".$imginfos[1]."'} $iurl {/missra:img}";
                }
            }
        }
...
function GetFormItemValue($ctag, $fvalue, $admintype='admin', $fieldname='')
{
    ...
    else if($ctag->GetAtt("type")=='stepselect')
    {
        ...
        $myformItem .= "<script type='text/javascript' src='{$cmspath}data/enums/{$fieldname}.js'></script>\r\n";
        $myformItem .= "<script type='text/javascript'>MakeTopSelect('$fieldname', $fvalue);</script>\r\n";
        $formitem = str_replace('~name~', $ctag->GetAtt('itemname'), $formitem);
        $formitem = str_replace('~form~', $myformItem, $formitem);
        return $formitem;
    }
