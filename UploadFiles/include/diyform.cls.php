...
class diyform
{
    ...
    function __construct($diyid){
        ...
        $this->listTemplate = $diyinfo['listtemplate'] != '' && file_exists(MITEMPLATE.'/plus/'.$diyinfo['listtemplate']) ? $diyinfo['listtemplate'] : 'list_diyform.htm';
        $this->viewTemplate = $diyinfo['viewtemplate'] != '' && file_exists(MITEMPLATE.'/plus/'.$diyinfo['viewtemplate']) ? $diyinfo['viewtemplate'] : 'view_diyform.htm';;
        $this->postTemplate = $diyinfo['posttemplate'] != '' && file_exists(MITEMPLATE.'/plus/'.$diyinfo['posttemplate']) ? $diyinfo['posttemplate'] : 'post_diyform.htm';;
    }
    ...
    function getForm($type = 'post', $value = '', $admintype='diy')
    {
        ...
        if(is_array($dtp->CTags))
        {
            foreach($dtp->CTags as $tagid=>$tag)
            {
                if($tag->GetAtt('autofield'))
                {
                    ...
                    {
                        $formstring .= $func($tag,mi_htmlspecialchars($value[$tag->GetName()],ENT_QUOTES),$admintype);
                    }
        ...
        $formstring .= "<input type=\"hidden\" name=\"mi_fields\" value=\"".$formfields."\" />\n";
        $formstring .= "<input type=\"hidden\" name=\"mi_fieldshash\" value=\"".md5($formfields.$cfg_cookie_encode)."\" />";
        return $formstring;
    }
