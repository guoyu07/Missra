 ...
 * 文档小助手
 *
 * @version        $Id: channelunit.helper.php
 * @package        Mi.Helpers
...
function MakeOneTag(&$dtp, &$refObj, $parfield='Y')
{
    ...
    foreach($dtp->CTags as $tagid=>$ctag)
    {
        ...
        if(in_array($tagname,$alltags))
        {
            ...
            {
                if(DEBUG_LEVEL) echo 'Missra Error:Tag disabled:"'.$tagname.'" <a href="http://help.missra.com/install-use/apply/2013/0711/2324.html" target="_blank">more...</a>!';
                continue;
            }
            if (DEBUG_LEVEL==TRUE) {
                $ttt1 = ExecTime();
            }
            $filename = MIINC.'/taglib/'.$tagname.'.lib.php';
...
function GetBookUrl($bid,$title,$gdir=0)
{
    global $cfg_cmspath;
    $bookurl = $gdir==1 ?
    "{$cfg_cmspath}/book/".MiID2Dir($bid) : "{$cfg_cmspath}/book/".MiID2Dir($bid).'/'.GetPinyin($title).'-'.$bid.'.html';
    return $bookurl;
}
...
function GetCacheBlock($cacheid)
{
    ...
    $cachefile = MIDATA.'/cache/'.$cacheid.'.inc';
