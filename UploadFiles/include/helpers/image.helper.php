...
if ( ! function_exists('WaterImg'))
{
    ...
    {
        include(MIDATA.'/mark/inc_photowatermark_config.php');
        require_once(MIINC.'/image.class.php');
        ...
        if($fromGo=='down' && $photo_markdown=='0')
        {
         ...
         $TRUEMarkimg = MIDATA.'/mark/'.$photo_markimg;
        ...
        if($photo_marktype == '2')
        {
            if(file_exists(MIDATA.'/mark/simhei.ttf'))
            {
                $cfg_watermarktext['fontpath'] =  MIDATA.'/mark/simhei.ttf';
            }
            else
            {
                return ;
