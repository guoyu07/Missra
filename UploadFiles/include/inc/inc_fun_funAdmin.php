 ...
 * 管理员后台基本函数
 *
 * @version        $Id:inc_fun_funAdmin.php
...
function SpGetPinyin($str, $ishead=0, $isclose=1)
{
    ...
    if(count($pinyins) == 0)
    {
        $fp = fopen(MIINC.'/data/pinyin.dat', 'r');
        while(!feof($fp))
        {
            $line = trim(fgets($fp));
...
function SpGetEditor($fname,$fvalue,$nheight="350",$etype="Basic",$gtype="print",$isfullpage="false",$bbcode=false)
{
    ...
    {
        require_once(MIINC.'/FCKeditor/fckeditor.php');
    ...
    {
        require_once(MIINC.'/ckeditor/ckeditor.php');
        $CKEditor = new CKEditor();
        $CKEditor->basePath = $GLOBALS['cfg_cmspath'].'/include/ckeditor/' ;
        $config = $events = array();
        $config['extraPlugins'] = 'mipage,multipic,addon,dewplayer';
		if($bbcode)
		{
			...
			// 获取表情信息
			require_once(MIDATA.'/smiley.data.php');
			$jsscript = array();
    ...
    } else  if($GLOBALS['cfg_html_editor']=='kindeditor'){ //kindeditor//
		...
		if($etype == 'Member' || $etype == 'MemberLit' || $etype == 'Diy' || $etype == 'Feedback')
		{
			...
			$extendconfig = 'allowImageUpload : true,';
			$extendconfig .= 'allowFlashUpload : true,';
			$extendconfig .= 'allowMediaUpload : true,';
			$extendconfig .= 'allowFileUpload : true,';
		}
		...
		$items['Small'] = $items['MemberLit'] = $items['Diy']= $items['Feedback']= "[
...
EOT;
	...
	//kindeditor//
        ...
        /*
        // ------------------------------------------------------------------------
        // 当前版本,暂时取消misshtml编辑器的支持
        // ------------------------------------------------------------------------
        require_once(MIINC.'/htmledit/mi_editor.php');
        $ded = new MiEditor($fname);
...
function SpGetNewInfo()
{
	...
	$offUrl = "http://newver.api.missra.com/index.php?version={$cfg_version}&formurl={$nurl}&phpver={$phpv}&os={$sp_os}&mysqlver={$mysql_ver}{$add_query}";
    return $offUrl;
}
