...
class MemberLogin
{
    ...
    function __construct($kptime = -1, $cache=FALSE)
    {
        ...
        $formcache = FALSE;
        $this->M_ID = $this->GetNum(GetCookie("MiUserID"));
        $this->M_LoginTime = GetCookie("MiLoginTime");
        $this->fields = array();
        $this->isAdmin = FALSE;
        if(empty($this->M_ID))
        {
            ...
            if(is_array($this->fields)){
                ...
                if(time() - $this->M_LoginTime > 3600)
                {
                    ...
                    PutCookie("MiLoginTime",time(),$this->M_KeepTime);
                }
    ...
    function PutLoginInfo($uid, $logintime=0)
    {
        ...
        {
            PutCookie('MiUserID',$uid,$this->M_KeepTime);
            PutCookie('MiLoginTime',$this->M_LoginTime,$this->M_KeepTime);
        }
        else
        {
            PutCookie('MiUserID',$uid);
            PutCookie('MiLoginTime',$this->M_LoginTime);
        }
    }
    ...
    function RecordFeeds($type, $title, $note, $aid)
    {
        ...
        if (in_array($type,array('add','addsoft','feedback','addfriends','stow'))){
            $ntime = time();
            $title = mi_htmlspecialchars(cn_substrR($title,255));
            if(in_array($type,array('add','addsoft','feedback','stow')))
            {
                ...
                $title = mi_htmlspecialchars($rcdtype[$type].$arcrul, ENT_QUOTES);
            } else if ($type == 'addfriends')
            {
                //添加好友处理
                $arcrul = " <a href='/member/index.php?uid=".$aid."'>".$aid."</a>";
                $title = mi_htmlspecialchars(' 与'. $arcrul."成为好友", ENT_QUOTES);
            }
            $note = Html2Text($note);
