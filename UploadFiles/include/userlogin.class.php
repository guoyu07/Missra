 ...
 * 管理员登陆类
 *
 * @version        $Id: userlogin.class.php
...
{
    $cacheFile = MIDATA.'/cache/addon-'.session_id().'.inc';
...
class userLogin
{
    ...
    var $userChannel = '';
    var $userPurview = '';
    var $keepUserIDTag = 'mi_admin_id';
    var $keepUserTypeTag = 'mi_admin_type';
    var $keepUserChannelTag = 'mi_admin_channel';
    var $keepUserNameTag = 'mi_admin_name';
    var $keepUserPurviewTag = 'mi_admin_purview';
    var $keepAdminStyleTag = 'mi_admin_style';
    var $adminStyle = 'missra';
    ...
    function keepUser()
    {
        if($this->userID != '' && $this->userType != '')
        {
            ...
            if(empty($adminstyle)) $adminstyle = 'missra';
            ...
            PutCookie('MiUserID', $this->userID, 3600 * 24, '/');
            PutCookie('MiLoginTime', time(), 3600 * 24, '/');
    ...
    {
        //$this->userChannel
        $cacheFile = MIDATA.'/cache/admincat_'.$this->userID.'.inc';
        //管理员管理的频道列表
    ...
    function exitUser()
    {
        ...
        DropCookie('missAdmindir');
        DropCookie('MiUserID');
        DropCookie('MiLoginTime');
        $_SESSION = array();
    }
...
function GetSonIdsUL($id, $channel=0, $addthis=TRUE)
{
    ...
    if( !is_array($cfg_Cs) )
    {
        require_once(MIDATA."/cache/inc_catalog_base.inc");
    }
