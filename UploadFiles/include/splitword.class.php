 ...
 *
 * @version        $Id: splitword.class.php
...
class SplitWord
{
    ...
    function __construct($source_charset='utf-8', $target_charset='utf-8', $load_all=TRUE, $source='')
    {
        ...
        if(file_exists(MIINC.'/'.$this->mainDicFile)) $this->isUnpacked = TRUE;
    ...
    */
    ...
    {
		$this->addonDicFile = MIINC.'/'.$this->addonDicFile;
		$this->mainDicFile = MIINC.'/'.$this->mainDicFile;
		$this->mainDicFileZip = MIINC.'/'.$this->mainDicFileZip;
	...
	function InportDict( $targetfile )
     {
     	if(!ini_set('memory_limit', '128M'))
			exit('设置内存错误，请到missra官网下载解压版的base_dic_full.dic!');
     	require_once(MIINC.'/zip.class.php');
     	$zip = new zip();
     	//echo $targetfile;
     	$unpackagefile = array_keys($zip->Extract($targetfile,MIINC.'/data/'));
     	//exit();
     	$this->MakeDict(MIINC.'/data/'.$unpackagefile[0]);
     	unlink(MIINC.'/data/'.$unpackagefile[0]);
     	return true;
     }
}
