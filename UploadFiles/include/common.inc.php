...
define('MIINC', str_replace("\\", '/', dirname(__FILE__) ) );
define('MIROOT', str_replace("\\", '/', substr(MIINC,0,-8) ) );
define('MIDATA', MIROOT.'/data');
define('MIMEMBER', MIROOT.'/member');
define('MITEMPLATE', MIROOT.'/templets');

// ------------------------------------------------------------------------
define('MIMODEL', './model');
define('MICONTROL', './control');
define('MIAPPTPL', './templates');
...
if (!defined('MIREQUEST')) {
    //检查和注册外部提交的变量   (修改登录时相关过滤)
...
//系统配置参数
require_once(MIDATA."/config.cache.inc.php");

//Session保存路径
$enkey = substr(md5(substr($cfg_cookie_encode,0,5)),0,10);
$sessSavePath = MIDATA."/sessions_{$enkey}";
...
if($_FILES) {
    require_once(MIINC.'/uploadsafe.inc.php');
}

//数据库配置文件
require_once(MIDATA.'/common.inc.php');

//载入系统验证安全配置
if(file_exists(MIDATA.'/safe/inc_safe_config.php')){
    require_once(MIDATA.'/safe/inc_safe_config.php');
...
//站点根目录
$cfg_basedir = preg_replace('#'.$cfg_cmspath.'\/include$#i', '', MIINC);
if($cfg_multi_site == 'Y') {
...
$cfg_version = 'V1.0-Release';
$cfg_soft_lang = 'utf-8';
$cfg_soft_public = 'base';

$cfg_softname = 'Missra内容管理系统';
$cfg_soft_enname = 'Missra';
$cfg_soft_devteam = 'Missra官方团队';
...
function __autoload($classname) {
    ...
    $libclassfile = $classname.'.class.php';
        if ( is_file ( MIINC.'/'.$libclassfile ) ) {
            require MIINC.'/'.$libclassfile;
        } else if( is_file ( MIMODEL.'/'.$classfile ) ) {
            require MIMODEL.'/'.$classfile;
        } else {
            if (DEBUG_LEVEL === TRUE) {
                echo '<pre>';
				echo $classname.'类找不到';
...
if ($GLOBALS['cfg_mysql_type'] == 'mysqli' && function_exists("mysqli_init") || !function_exists('mysql_connect')) {
    require_once(MIINC.'/misqli.class.php');
} else {
    require_once(MIINC.'/misql.class.php');
}
...
// 模块MVC框架需要的控制器和模型基类
require_once(MIINC.'/control.class.php');
require_once(MIINC.'/model.class.php');

//载入小助手配置,并对其进行默认初始化
if(file_exists(MIDATA.'/helper.inc.php')) {
    require_once(MIDATA.'/helper.inc.php');
