 ...
 * 文档类
 *
 * @version        $Id: arc.archives.class.php
...
require_once(MIINC."/downmix.inc.php");
...
class Archives
{
    ...
    {
        //读取附加表信息，并把附加表的资料经过编译处理后导入到$this->Fields中，以方便在模板中用 {missra:field name='fieldname' /} 标记统一调用
        if($this->ChannelUnit->ChannelInfos['addtable']!='')
        {
        ...
        if (isset($this->Fields['litpic']))
        {
            ...
            //模板里直接使用{missra:field name='image'/}获取缩略图
        ...
        if (isset($this->Fields['voteid']) && !empty($this->Fields['voteid']))
        {
            ...
            $this->Fields['vote'] = "<script type='text/javascript' src='{$GLOBALS['cfg_cmspath']}/data/vote/vote_{$voteid}.js'></script>";
            if ($GLOBALS['cfg_multi_site'] == 'Y')
            {
                $this->Fields['vote'] = "<script type='text/javascript' src='{$GLOBALS['cfg_mainsite']}/data/vote/vote_{$voteid}.js'></script>";
            }
        }
        ...
        else
        {
            for($i=1;$i<=$this->TotalPage;$i++)
            {
                ...
                if($cfg_remote_site=='Y' && $isremote == 1)
                {
                    ...
                    //分析远程文件路径
                    $remotefile = str_replace(MIROOT, '', $TRUEfilename);
    ...
    function GetPagebreakDM($totalPage, $nowPage, $aid)
    {
        ...
        for($i=1;$i<=$totalPage;$i++)
        {
            if($i==1)
            {
                ...
                {
                    $PageList.="<li class=\"active thisclass\"><a>1</a></li>";
                }
            }
            else
            {
                ...
                {
                    $PageList.="<li class=\"active thisclass\"><a href='#'>{$n}</a></li>";
                }
            }
        }
        if($lPage <= $totalPage)
    ...
    function GetPagebreak($totalPage, $nowPage, $aid)
    {
        ...
        for($i=1;$i<=$totalPage;$i++)
        {
            if($i==1)
            {
                ...
                {
                    $PageList.="<li class=\"active thisclass\"><a href='#'>1</a></li>";
                }
            }
            else
            {
    ...
    function GetPageTitlesDM($styleName, $pageNo)
    {
        ...
        {
            $revalue = "<select id='mipagetitles' onchange='location.href=this.options[this.selectedIndex].value;'>\r\n";
            foreach($this->SplitTitles as $k=>$v)
            {
                if($i==1)
                {
