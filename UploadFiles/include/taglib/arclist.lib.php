...
<demo>
{missra:arclist  flag='h' typeid='' row='' col='' titlelen='' infolen='' imgwidth='' imgheight='' listtype='' orderby='' keyword='' limit='0,1'}
        ...
        for($j=0; $j<$col; $j++)
        {
            ...
            if($row = $dsql->GetArray("al"))
            {
				...
				if($currentstyle && $row['id']==$arcid){
					$currentstyle = str_replace('~arclink~', $row['filename'], $currentstyle);
					$row['currentstyle'] = str_replace('~title~', $row['title'], $currentstyle);
				}
