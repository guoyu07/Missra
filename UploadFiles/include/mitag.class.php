...
class MiTag {
    ...
    var $EndPos=0;        //标记结束位置
    var $CAttribute="";   //标记属性描述,即是class MiAttribute
    var $TagValue="";     //标记的值
    var $TagID = 0;
...
/**
 * MiTagParse Missra玫莎模板类
 * function c____MiTagParse();
 *
 * @package          MiTagParse
 * @subpackage       Mi.Libraries
 * @link             http://www.missra.com
...
class MiTagParse
{
        ...
        if (defined('MIDISFUN')) {
            ...
            foreach ($tokens as $token)
            {
                if (is_array($token))
                {
                    ...
                    {
                       $errmsg = 'Missra Error:function disabled "'.$token[1].'" <a href="http://www.missra.com/" target="_blank">more...</a>';
                       return FALSE;
                    }
                }
            }
    ...
    function LoadCache($filename)
    {
        ...
        $cachedir = MIROOT.$cfg_tplcache_dir;
        ...
        if( isset($z) && is_array($z) )
        {
            foreach($z as $k=>$v)
            {
                $this->Count++;
                $ctag = new MiTag();
                $ctag->CAttribute = new MiAttribute();
    ...
    function LoadSource($str)
    {
        ...
        */
        //优化模板字符串存取读取方式
        $this->taghashfile = $filename = MIDATA.'/tplcache/'.md5($str).'.inc';
        if( !is_file($filename) )
        {
    ...
    function RunPHP(&$refObj, $i)
    {
        ...
        {
            $MiMeValue = $this->CTags[$i]->TagValue;
            $phpcode = $refObj->GetInnerText();
        }
        $phpcode = preg_replace("/'@me'|\"@me\"|@me/i", '$MiMeValue', $phpcode);
        @eval($phpcode); //or die("<xmp>$phpcode</xmp>");

        $this->CTags[$i]->TagValue = $MiMeValue;
        $this->CTags[$i]->IsReplace = TRUE;
    }
    ...
    {
        $fp = @fopen($filename,"w") or die("MiTag Engine Create File False");
        ...
        for($i=0; $i < $sourceLen; $i++)
        {
            ...
            */
            ...
            if($tTagName!='')
            {
                ...
                //found '/}' and found '{/missra:'
                else
                {
                    //if '/}' more near '{missra:'、'{/missra:' , end tag is '/}', else is '{/missra:'
                    if($e1 < $e2 &&  $e1 < $e3 )
                    {
                        $endPos = $e1;
                        $elen = $endPos + strlen($eTagEndWord);
    ...
    {
        $MiFieldValue = $fieldvalue;
        $functionname = str_replace("{\"","[\"",$functionname);
        $functionname = str_replace("\"}","\"]",$functionname);
        $functionname = preg_replace("/'@me'|\"@me\"|@me/i",'$MiFieldValue',$functionname);
        $functionname = "\$MiFieldValue = ".$functionname;
        @eval($functionname.";"); //or die("<xmp>$functionname</xmp>");
        if(empty($MiFieldValue))
        {
            return '';
        }
        else
    ...
    function IncludeFile($filename, $ismake='no')
    {
        ...
        if( file_exists(MIROOT."/templets/".$filename) )
        {
            $okfile = MIROOT."/templets/".$filename;
        }
        else if(file_exists(MIROOT.'/templets/'.$cfg_df_style.'/'.$filename) )
        {
            $okfile = MIROOT.'/templets/'.$cfg_df_style.'/'.$filename;
        }
        else
        {
...
/**********************************************
//class MiAttribute Miss模板标记属性集合
function c____MiAttribute();
**********************************************/
//属性的数据描述
class MiAttribute
{
...
function c____MiAttributeParse();
********************************/
class MiAttributeParse
{
    var $sourceString = "";
    ...
    {
        $this->cAttributes = new MiAttribute();
        $strLen = 0;
