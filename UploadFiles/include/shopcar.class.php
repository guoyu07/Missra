<?php
define("MI_ItemEcode",'Shop_Mi_');//识别购物车Cookie前缀,非开发人员请不要随意更改!
/**
 * 购物车类
 *
 * @version        $Id: shopcar.class.php
...
class MemberShops
{
    ...
    {
        $this->productsId = MI_ItemEcode.$id;
    ...
    function clearItem()
    {
        ...
        {
            if(preg_match('/'.MI_ItemEcode.'/', $key))
            {
    ...
    function getItems()
    {
        ...
        foreach($_COOKIE as $key => $vals)
        {
            if(preg_match("#".MI_ItemEcode."#", $key) && preg_match("#[^_0-9a-z]#", $key))
            {
    ...
    function priceCount()
    {
        ...
        foreach($_COOKIE as $key => $vals)
        {
            if(preg_match("/".MI_ItemEcode."/", $key))
            {
                $Products = $this->getOneItem(str_replace(MI_ItemEcode,"",$key));
