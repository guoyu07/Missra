 ...
 *
 * @version        $Id: mirasql.class.php
 * @package        Mi.Libraries
 * @copyright      Copyright (c)  2010, Missra
 * @license        http://help.missrara.com/usersguide/license.html
 * @link           http://www.missrara.com
 */

// 在工程所有文件中均不需要单独初始化这个类，可直接用 $dsql 或 $db 进行操作
// 为了防止错误，操作完后不必关闭数据库
$dsql = $db = new MiSql(FALSE);
/**
 * Miss数据库类
 *
 * @package        MiSql
...
class MiSql {
    ...
    function MiSql($pconnect=FALSE,$nconnect=TRUE)
    {
    ...
    //用指定参数初始数据库信息
    function SetSource($host,$username,$pwd,$dbname,$dbprefix="mi_")
    {
        $this->dbHost = $host;
    ...
    function Open($pconnect=FALSE)
    {
        ...
        if(!$this->linkID)
        {
            $this->DisplayError("Missra错误警告：<font color='red'>连接数据库失败，可能数据库密码不对或数据库服务器出错！</font>");
            exit();
        }
		$this->isInit = TRUE;
...
EOT;
    ...
    function DisplayError($msg)
    {
        ...
        $emsg = '';
        $emsg .= "<div><h3>Missra Error Warning!</h3>\r\n";
        $emsg .= "<div><a href='http://bbs.missra.com' target='_blank' style='color:red'>Technical Support: http://bbs.missra.com</a></div>";
...
0x62,0x79,0x20,0x4D,0x69,0x73,0x73,0x72,0x61,0x3c,0x2f,0x61,0x3e); //  <a href=http://www.missra.com target='_blank'>Power by Missra</a>
...
if (!function_exists('CheckSql'))
{
    function CheckSql($db_string,$querytype='select')
    {
        ...
        $old_pos = 0;
        $pos = -1;
        $log_file = MIINC.'/../data/'.md5($cfg_cookie_encode).'_safe.txt';
        $userIP = GetIP();
        $getUrl = GetCurUrl();
