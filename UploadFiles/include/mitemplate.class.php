 ...
 * 模板引擎文件
 *
 * @version        $Id: mitemplate.class.php
...
function MakePublicTag($atts=array(),$refObj='',$fields=array())
{
    ...
    $plusfile = MIINC.'/tpllib/plus_'.$atts['tagname'].'.php';
...
class MiTemplate
{
    ...
    var $refObj = '';
    var $makeLoop = 0;
    var $tagStartWord =  '{missra:';
    var $fullTagEndWord =  '{/missra:';
    var $sTagEndWord = '/}';
    var $tagEndWord = '}';
    var $tpCfgs = array();
    ...
    function __construct($templatedir='',$refDir='')
    {
        ...
        if($templatedir=='')
        {
            $this->templateDir = MIROOT.'/templates';
        }
        else
        {
        ...
        $this->cacheDir = MIROOT.$GLOBALS['cfg_tplcache_dir'];
    }

    //构造函数,兼容PHP4
    function MiTemplate($templatedir='',$refDir='')
    {
     ...
     * @return    void
     */
    function SetTagStyle($ts='{missra:',$ftend='{/missra:',$stend='/}',$tend='}')
    {
    ...
    function CompilerInclude($filename, $isload=TRUE)
    {
        ...
        if($okfile=='') return 0;
        if( !$isload ) return 1;
        $itpl = new MiTemplate($this->templateDir);
