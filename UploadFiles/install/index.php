...
$s_lang = 'utf-8';
$errmsg = '';
$dfDbname = 'missra';	//默认数据库名字
$install_demo_name = 'missraDemo.txt';	//示例数据
$insLockfile = dirname(__FILE__).'/install_lock.txt';
$moduleCacheFile = dirname(__FILE__).'/modules.tmp.inc';

define('MIINC',dirname(__FILE__).'/../include');
define('MIDATA',dirname(__FILE__).'/../data');
define('MIROOT',preg_replace("#[\\\\\/]install#", '', dirname(__FILE__)));
header("Content-Type: text/html; charset={$s_lang}");

require_once(MIROOT.'/install/install.inc.php');
require_once(MIINC.'/zip.class.php');
...
else if($step==4)
{
    ...
    @chmod(MIDATA,0777);
    $fp = fopen(MIDATA."/common.inc.php","w") or die("<script>alert('写入配置失败，请检查../data目录是否可写入！');history.go(-1);</script>");
    ...
    $fp = fopen(MIDATA.'/config.cache.inc.php','w');
    fwrite($fp,$configStr2);
    fclose($fp);

    $fp = fopen(MIDATA.'/config.cache.bak.php','w');
    fwrite($fp,$configStr2);
    fclose($fp);
...
else if($step==11)
{
	...
	$rmurl = $updateHost."missra/demodata.{$s_lang}.txt";
